...
client = phub.Client('allashevtcova@yandex.ru', '1qa2ws3ed', delay = 0.3)
videoUrl = set()
...
def input():
    with open(inputF, 'r+', encoding='utf-8') as FR,open('tmp.json', 'w', encoding='utf-8') as OUT:
        ...
        with open(inputF, 'a', encoding='utf-8') as FW:
                for video in suppress(client.account.watched, phub.errors.RegexError):
                    try:
